(function () {
  ...
  const AR = {
    ...
    priceMonths: "أشهر السعر الرسمي", calendar: "التقويم", rating: "التقييم الموثق", licence: "معلومات الإعلان", ready: "جاهز", missing: "ناقص", staff: "بيانات تحتاج إكمال", background: "عوائق المصدر الحي", none: "لا توجد عوائق حالياً.", showcaseEligibleWithGaps: "تظهر مع بيانات ناقصة",
  ...
  function renderRows() {
    ...
    rows.forEach(function (row) {
      ...
      const reviewChip = node("span", { className: "status-chip " + (row.review_ready ? "ready" : "warning"), text: reviewCopy });
      const statusStack = node("div", { className: "status-stack" }); add(statusStack, chip, reviewChip);
      ...
      add(item, picture, name, progress, statusStack, gap, button); root.appendChild(item);
    });
  }
  ...
  function renderShowcases() {
    ...
    state.showcases.forEach(function (record) {
      ...
      if (record.eligible_with_gaps_count) meta.appendChild(node("span", { className: "showcase-blocked", text: record.eligible_with_gaps_count + " " + text("showcaseEligibleWithGaps") }));
  ...
  function renderSources(root) {
    ...
    const reviews = source.reviews || {}, reviewCount = Number(reviews.rating_count) || 0;
    add(cards, readiness(text("priceMonths"), months.join("، "), months.length > 0), readiness(text("calendar"), source.calendar && Object.keys(source.calendar).length, Boolean(source.calendar && Object.keys(source.calendar).length)), readiness(text("rating"), source.rating && source.rating.rating, Boolean(source.rating && source.rating.rating)), readiness(COPY[state.lang].reviewsReady.replace("{count}", reviewCount), reviews.rating, reviewCount > 0), readiness(text("images"), source.image_count, source.image_count >= 3), readiness(text("licence"), source.licence_present ? text("ready") : "—", source.licence_present)); part.appendChild(cards); root.appendChild(part);
  }
  ...
  async function saveProfile(preview) {
    try {
      const saved = await api("/api/monthly/ops/listing/" + encodeURIComponent(state.listing.id) + "/draft", { method: "POST", body: JSON.stringify({ revision: state.listing.draft_revision, profile: buildProfilePayload(state.profile) }) });
      state.listing.draft_revision = saved.draft_revision;
      id("survey-save-status").textContent = text("saved");
      empty(id("survey-error-summary"));
      id("survey-error-summary").hidden = true;
      const refreshed = await api("/api/monthly/ops/listing/" + encodeURIComponent(state.listing.id));
      state.listing = refreshed;
      state.profile = clone(refreshed.prefill);
      delete state.profile.sources;
      delete state.profile.source_readiness;
      if (preview) {
