...
from . import audit, html as html_mod, tokens
...
def _render_in_browser(pages_html_path, story_html_path, pdf_path, png_path, run_audit):
    from playwright.sync_api import sync_playwright
...
def render(payload, art_map, out_dir, issue_no, run_audit=True, guard_ctx=None):
    ...
    try:
        res = _pool.submit(_render_in_browser, paths["html"], paths["story_html"], paths["pdf"], paths["png"], run_audit).result()
