...
const T = {
  ...
  en:{dir:'ltr',
    ...
    rvw:{
      ...
      bulk:'🤖 Analyze all (negatives first)',
...
// New (Part 2) full option lists.
const WR_UPDATES = ["لا يوجد تحديث","تم الديب كلين","تم تغيير المفارش","تم إصلاح المكيف","تم إصلاح السخان","تم إصلاح الكهرباء","تم حل تسريب المياه","تم رش الحشرات","تم تزويد المستلزمات","تم تغيير الأقفال/الكود","تم تحسين الواي فاي","تم غسيل الكنب","تمت إعادة التصميم","تم التواصل مع المالك"];
const WR_SUPERVISOR = ["لا يوجد إجراء","زيارة ميدانية","إرسال فني","إرسال شركة تنظيف","طلب قطع غيار","متابعة مع المورد","تصعيد للإدارة","إغلاق الشقة مؤقتاً","جدولة ديب كلين","تأكيد الحل ميدانياً"];
const WR_SEVERITY = [["low","🟢 منخفض"],["med","🟡 متوسط"],["high","🔴 عالي"]];
...
function _renderWeeklyBody(){
  ...
  for(const r of items){
    const sm = r.summary||{};
    let badges = '';
    if(sm.unresolved) badges += '<span style="background:#fee2e2;color:#991b1b;border-radius:99px;padding:1px 8px;font-size:10px;margin-inline-start:5px">⚠ '+sm.unresolved+' غير محلول</span>';
    if(sm.carried) badges += '<span style="background:#fef3c7;color:#92400e;border-radius:99px;padding:1px 8px;font-size:10px;margin-inline-start:5px">↪ '+sm.carried+' مُرحّل</span>';
    if(sm.resolved) badges += '<span style="background:#d1fae5;color:#065f46;border-radius:99px;padding:1px 8px;font-size:10px;margin-inline-start:5px">✓ '+sm.resolved+'</span>';
    h += '<div onclick="openWeeklyEditor(&#39;'+esc(r.id)+'&#39;)" style="background:var(--surface-2);padding:13px 14px;border-radius:12px;border:1px solid var(--border);cursor:pointer;display:flex;justify-content:space-between;align-items:center" onmouseover="this.style.borderColor=&#39;var(--gold)&#39;" onmouseout="this.style.borderColor=&#39;var(--border)&#39;">'
      ...
      + '<div class="muted" style="font-size:11.5px;margin-top:3px">'+(r.apt_count||0)+' شقة'+badges+'</div></div>'
...
async function openWeeklyEditor(rid){
  ...
  if(!r){
    const today = new Date().toISOString().slice(0,10);
    r = {id:null, employee:'', date:today, apartments:[], highlights:'', unusual:''};
    // New report → pre-list every apartment, auto-enriched (reviews, tickets,
    // occupancy) and with last week's unresolved issues carried over.
    try {
      ...
      r.apartments = (tpl.apartments||[]).map(function(a){
        return {name:a.name, listing_id:a.listing_id, common_issues:a.common_issues||[],
                ...
                issues:(a.issues||[]), new_reviews:a.new_reviews||0,
...
function _renderWeeklyEditor(){
  ...
  const html =
    ...
    +     '<textarea oninput="if(_wrDraft)_wrDraft.unusual=this.value" rows="2" placeholder="أي ملاحظة خارجة عن المعتاد…" style="width:100%;padding:9px;margin-top:4px;background:var(--surface-2);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:12.5px;resize:vertical">'+esc(r.unusual||'')+'</textarea></div>'
    + '</div>'
...
function _wrComputeSummary(apts){
  apts = apts||[];
  function openIssues(a){ return (a.issues||[]).filter(function(it){ return it && !it.resolved; }); }
  let withIssue=0, resolved=0, carried=0, high=0, occupied=0;
  apts.forEach(function(a){
    const oi = openIssues(a);
    const hasChk = Object.keys(a.checks||{}).some(function(k){ return a.checks[k]==='issue'; });
    const hasRisk = (a.risks||[]).some(function(r){ return r!=='لا يوجد خطر' && r!=='No risk'; });
  ...
  return {total:apts.length, withIssue:withIssue, resolved:resolved,
          unresolved:Math.max(0,withIssue-resolved), carried:carried, high:high, occupied:occupied};
}
function _wrSummaryHtml(apts, r){
  const s = _wrComputeSummary(apts);
  function pill(lbl,val,col){ return '<div style="background:var(--surface-2);border:1px solid var(--border);border-radius:10px;padding:7px 10px;text-align:center;min-width:62px"><div style="font-size:17px;font-weight:800;color:'+(col||'var(--text)')+'">'+val+'</div><div class="muted" style="font-size:9.5px">'+lbl+'</div></div>'; }
  let banner = '';
  if(r && r._carried_from){
    banner = '<div style="background:linear-gradient(135deg,#fff3cd,var(--surface-2));border:1px solid #f0ad4e;border-radius:8px;padding:7px 11px;margin-bottom:8px;font-size:11.5px;color:#92400e">↪ تم ترحيل المشاكل غير المحلولة من تقرير '+esc(r._carried_from)+' (مع عدّاد الأسابيع)</div>';
  }
  return banner
    + '<div style="display:flex;gap:7px;flex-wrap:wrap;margin-bottom:14px">'
    + pill('شقة', s.total)
    + pill('فيها مشاكل', s.withIssue, s.withIssue?'#d97706':'var(--text)')
    + pill('غير محلول', s.unresolved, s.unresolved?'#c62828':'#16a34a')
    + pill('تم حلها', s.resolved, '#16a34a')
    + pill('مُرحّلة', s.carried, s.carried?'#c62828':'var(--text)')
    + pill('خطورة عالية', s.high, s.high?'#c62828':'var(--text)')
    + pill('مؤجّرة الآن', s.occupied, '#2563eb')
    + '</div>';
}
function _wrRenderApt(i, apt){
  ...
  chk += '</div></div>';
  // Auto-pulled facts strip (reviews / tickets / occupancy)
  const nr = apt.new_reviews||0; const ot = apt.open_tickets||[];
  function fact(txt,bg,col){ return '<span style="background:'+bg+';color:'+col+';border-radius:99px;padding:2px 10px;font-size:10.5px;font-weight:600">'+txt+'</span>'; }
  let auto = '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px">'
    + (apt.occupied ? fact('🟢 مؤجّرة الآن','#d1fae5','#065f46') : fact('⚪ شاغرة الآن','var(--surface)','var(--text-2)'))
    + (nr ? fact('⭐ '+nr+' مراجعة جديدة','#fef3c7','#92400e') : '')
    + (ot.length ? fact('🔧 '+ot.length+' تذكرة مفتوحة','#fee2e2','#991b1b') : '')
    + '</div>';
  if(ot.length){
    auto += '<div style="background:var(--surface);border:1px solid var(--border);border-radius:7px;padding:6px 9px;margin-bottom:10px">'
      + ot.slice(0,6).map(function(t){ return '<div style="font-size:11px">• '+esc(t.title||'')+' <span class="muted">('+esc(t.priority||'')+')</span></div>'; }).join('')
      + '</div>';
  }
  function other(key, ph){
    return '<input value="'+esc(apt[key]||'')+'" oninput="_wrSetField('+i+',\\''+key+'\\',this.value)" placeholder="'+ph+'" style="width:100%;margin-top:5px;padding:6px 9px;background:var(--surface);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:11.5px">';
  }
  return '<div style="background:var(--surface-2);padding:12px;border-radius:10px;margin-bottom:10px;border:1px solid var(--border)">'
    ...
    + '<div style="margin-bottom:6px"><div class="muted" style="font-size:10.5px;font-weight:600;margin-bottom:4px">التحديثات</div><div style="display:flex;flex-wrap:wrap;gap:4px">'+chips('updates', WR_UPDATES, apt.updates)+'</div>'+other('updates_other','أخرى — اكتب هنا…')+'</div>'
    ...
    + '<div style="margin-bottom:6px"><div class="muted" style="font-size:10.5px;font-weight:600;margin-bottom:4px">خطة الاستجابة</div><div style="display:flex;flex-wrap:wrap;gap:4px">'+chips('plan', WR_PLAN, apt.plan)+'</div>'+other('plan_other','أخرى — اكتب هنا…')+'</div>'
    + '<div style="margin-bottom:6px"><div class="muted" style="font-size:10.5px;font-weight:600;margin-bottom:4px">إجراء المشرف</div><div style="display:flex;flex-wrap:wrap;gap:4px">'+chips('supervisor', WR_SUPERVISOR, apt.supervisor)+'</div>'+other('supervisor_other','أخرى — اكتب هنا…')+'</div>'
    + _wrIssuesHtml(i, apt)
    + '</div></div>';
}
function _wrIssuesHtml(i, apt){
  ...
  let h = '<div style="margin-top:10px;border-top:1px dashed var(--border);padding-top:10px">'
    ...
    + '<div class="muted" style="font-size:10.5px;font-weight:700">🎯 مشاكل تحتاج متابعة (خطورة · مسؤول · تاريخ متابعة)</div>'
    + '<button onclick="_wrAddIssue('+i+')" class="btn ghost sm" style="padding:2px 10px;font-size:11px">+ مشكلة</button></div>';
  if(!issues.length){
    h += '<div class="muted" style="font-size:11px;padding:4px 2px">ما فيه مشاكل مفتوحة لهذي الشقة.</div>';
  }
  issues.forEach(function(it, j){
    const wk = it.weeks||1;
    const wkBadge = wk>1 ? '<span style="background:#fee2e2;color:#991b1b;border-radius:99px;padding:1px 8px;font-size:10px;font-weight:700">↪ مفتوحة منذ '+wk+' أسابيع</span>' : '<span style="background:var(--gold-tint);border-radius:99px;padding:1px 8px;font-size:10px">جديدة</span>';
    let sevSel = '<select onchange="_wrIssueSet('+i+','+j+',\\'severity\\',this.value)" style="padding:4px 8px;background:var(--surface);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:11px">';
    WR_SEVERITY.forEach(function(sv){ sevSel += '<option value="'+sv[0]+'"'+(it.severity===sv[0]?' selected':'')+'>'+sv[1]+'</option>'; });
    sevSel += '</select>';
    h += '<div style="background:'+(it.resolved?'var(--surface)':'var(--surface-2)')+';border:1px solid '+(it.severity==='high'?'#ef4444':'var(--border)')+';border-radius:8px;padding:8px;margin-bottom:6px'+(it.resolved?';opacity:.6':'')+'">'
      + '<div style="display:flex;gap:6px;align-items:center;margin-bottom:5px;flex-wrap:wrap">'+wkBadge+sevSel
...
function _wrSetField(i, key, val){ if(_wrDraft && _wrDraft.apartments[i]) _wrDraft.apartments[i][key] = val; }
...
function _wrIssueSet(i, j, key, val, noRender){
  if(!_wrDraft || !_wrDraft.apartments[i]) return;
  const it = (_wrDraft.apartments[i].issues||[])[j]; if(!it) return;
  it[key] = val;
  if(!noRender) _renderWeeklyEditor();
}
function _wrIssueResolve(i, j){
  const it = (((_wrDraft||{}).apartments||[])[i]||{}).issues[j]; if(!it) return;
  it.resolved = !it.resolved; _renderWeeklyEditor();
}
function _wrIssueDel(i, j){
  const a = ((_wrDraft||{}).apartments||[])[i]; if(!a||!a.issues) return;
  a.issues.splice(j,1); _renderWeeklyEditor();
}
function _wrAddApt(){
  if(!_wrDraft) return;
...
function printWeekly(){
  if(!_wrDraft) return;
  const r = _wrDraft;
  const s = _wrComputeSummary(r.apartments||[]);
  const SEVTXT = {low:'منخفض',med:'متوسط',high:'عالي'};
  const SEVCOL = {low:'#16a34a',med:'#d97706',high:'#ef4444'};
  function _hasData(a){
    const c = a.checks||{};
    const anyCheck = Object.keys(c).some(k=>c[k]);
    return anyCheck || a.status || (a.risks||[]).length || (a.updates||[]).length || (a.challenges||[]).length
      || (a.plan||[]).length || (a.supervisor||[]).length || a.resolved || (a.issues||[]).length
      || a.risks_other || a.updates_other || a.plan_other || a.supervisor_other;
  }
  function chipRow(lbl, arr, extra, bg){
    const all = (arr||[]).slice(); if(extra) all.push(extra);
  ...
  (r.apartments||[]).filter(_hasData).forEach(function(a){
    ...
    const c = a.checks||{};
    const issueItems = Object.keys(c).filter(function(k){ return c[k]==='issue'; });
  ...
  const summaryHtml = '<div style="display:grid;grid-template-columns:repeat(6,1fr);gap:8px;margin-bottom:18px">'
  ...
  const w = window.open('','_blank');
  w.document.write('<!doctype html><html dir="rtl" lang="ar"><head><meta charset="utf-8"><title>Ouja — تقرير '+esc(r.employee||'')+'</title>'
    +'<style>*{box-sizing:border-box}body{font-family:Tahoma,Arial,sans-serif;background:#0e0e12;color:#dfe3e8;padding:34px;max-width:920px;margin:auto;line-height:1.6}'
...
async def _api_weekly_list(request):
    ...
    for r in items[:200]:
        out.append({
            ...
            "summary": r.get("summary") or _wr_summary(r.get("apartments") or []),
...
def _wr_is_resolved(apt):
    """True if this apartment row is fully resolved (so it should NOT carry over)."""
    res = (apt.get("resolved") or "").strip().lower()
    return res in ("نعم", "yes", "تم الحل", "resolved")

def _wr_open_issues(apt):
    """Structured issues on an apartment row that are still open (resolved != True)."""
    return [it for it in (apt.get("issues") or [])
            if isinstance(it, dict) and not it.get("resolved")]

def _wr_latest_prior(before_date):
    """The most recent saved report strictly before `before_date` (ISO yyyy-mm-dd)."""
    cands = [r for r in _weekly_reports.values()
             if (r.get("date") or "") and (r.get("date") or "") < (before_date or "9999")]
    cands.sort(key=lambda r: (r.get("date") or "", r.get("created_at") or ""), reverse=True)
    return cands[0] if cands else None

def _wr_summary(apts):
    """Auto-computed report header counts (§Part 2 'auto summary')."""
    apts = apts or []
    total = len(apts)
    with_issue = sum(1 for a in apts if _wr_open_issues(a)
                     or any(v == "issue" for v in (a.get("checks") or {}).values())
                     or any(r not in ("لا يوجد خطر", "No risk") for r in (a.get("risks") or [])))
    resolved = sum(1 for a in apts if _wr_is_resolved(a))
    carried = sum(1 for a in apts for it in _wr_open_issues(a) if (it.get("weeks") or 1) > 1)
    high = sum(1 for a in apts for it in _wr_open_issues(a) if (it.get("severity") == "high"))
    occupied = sum(1 for a in apts if a.get("occupied"))
    return {"total": total, "with_issue": with_issue, "resolved": resolved,
            "unresolved": max(0, with_issue - resolved), "carried": carried,
            "high": high, "occupied": occupied}

async def _api_weekly_template(request):
    """GET → all apartments pre-listed, each auto-enriched with: most-common review
    issues, NEW reviews in the last 7 days, currently-open maintenance tickets,
    ...
    try:
        ...
        listings = {}
    # --- current occupancy (one targeted in-house query) ---
    today = datetime.now(TZ).date()
    occ_lids = set()
    try:
        rows = await asyncio.to_thread(fetch_inhouse, today)
        for r in rows:
            a, d = _parse_date(r.get("arrivalDate")), _parse_date(r.get("departureDate"))
            if a and d and a <= today < d:
                occ_lids.add(str(r.get("listingMapId")))
    except Exception:
        pass
    # --- new reviews in the last 7 days, per listing ---
    wk_ago = (today - timedelta(days=7)).isoformat()
    new_rev = {}
    for rv in _reviews.values():
        lid = rv.get("listing_id")
        if lid is None:
            continue
        if (rv.get("date") or "") >= wk_ago:
            new_rev[str(lid)] = new_rev.get(str(lid), 0) + 1
    # --- open tickets per listing ---
    open_tk = {}
    for t in _tickets:
        if t.get("status") in ("open", "in_progress") and t.get("lid") is not None:
            open_tk.setdefault(str(t["lid"]), []).append(
                {"title": t.get("title") or "", "priority": t.get("priority") or "med",
                 "status": t.get("status")})
    # --- carry-over from the previous report ---
    prior = _wr_latest_prior(today.isoformat())
    carry_by_lid, carry_by_name = {}, {}
    if prior:
        for a in (prior.get("apartments") or []):
            opens = _wr_open_issues(a)
            if not opens:
                continue
            carried = []
            for it in opens:
                c = dict(it)
                c["weeks"] = int(c.get("weeks") or 1) + 1
                c["carried"] = True
                carried.append(c)
            if a.get("listing_id") is not None:
                carry_by_lid[str(a["listing_id"])] = carried
            if a.get("name"):
                carry_by_name[a["name"]] = carried

    def enrich(name, lid):
        k = str(lid) if lid is not None else None
        carried = (carry_by_lid.get(k) if k else None) or carry_by_name.get(name) or []
        return {
            "new_reviews": (new_rev.get(k, 0) if k else 0),
            "open_tickets": (open_tk.get(k, []) if k else []),
            "occupied": (k in occ_lids) if k else False,
            "issues": [dict(c) for c in carried],
        }
...
async def _api_weekly_save(request):
    ...
    # Report-level free-text fields (§Part 2)
    if "highlights" in b: r["highlights"] = (b.get("highlights") or "").strip()[:2000]
    if "unusual" in b:    r["unusual"]    = (b.get("unusual") or "").strip()[:2000]
    if isinstance(b.get("apartments"), list):
        ...
        for apt in b["apartments"]:
            ...
            # Structured, trackable issues (severity / assignee / follow-up / week count)
            issues = []
            for it in (apt.get("issues") or [])[:30]:
                if not isinstance(it, dict): continue
                ...
                issues.append({
                    "text":     txt,
                    "severity": sev if sev in ("low", "med", "high") else "med",
                    "assignee": (it.get("assignee") or "").strip()[:60],
                    "followup": (it.get("followup") or "").strip()[:10],
                    "weeks":    int(it.get("weeks") or 1),
            ...
            cleaned.append({
                ...
                "supervisor":[str(x)[:160] for x in (apt.get("supervisor") or [])][:20],
                "resolved":  (apt.get("resolved") or "").strip()[:40],
                "risks_other":      (apt.get("risks_other") or "").strip()[:200],
                "updates_other":    (apt.get("updates_other") or "").strip()[:200],
                "plan_other":       (apt.get("plan_other") or "").strip()[:200],
                "supervisor_other": (apt.get("supervisor_other") or "").strip()[:200],
                "comments":  [str(x)[:200] for x in (apt.get("comments") or [])][:20],
                "common_issues": [str(x)[:60] for x in (apt.get("common_issues") or [])][:10],
                "issues":    issues,
                "new_reviews": int(apt.get("new_reviews") or 0),
                "open_tickets": [
                    {"title": str(t.get("title", ""))[:120],
                     "priority": str(t.get("priority", "med"))[:10],
                     "status": str(t.get("status", ""))[:20]}
                    for t in (apt.get("open_tickets") or []) if isinstance(t, dict)][:20],
                "occupied":  bool(apt.get("occupied")),
                "checks":    checks,
            })
        r["apartments"] = cleaned
        r["summary"] = _wr_summary(cleaned)
